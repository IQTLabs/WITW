...
class Globals:
    ...
    img_mean = [0.485, 0.456, 0.406, 0.45, 0.45]
    img_std = [0.229, 0.224, 0.225, 0.22, 0.22]
    ...
    dataset_paths = {
        ...
        'witw': {
            'train':'./data4/train_scenes.csv',
            'test':'./data4/test_scenes.csv',
            'semantic':True
        }
    }
...
class ImagePairDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, dataset, csv_path, base_path=None, transform=None):
        ...
        self.semantic = Globals.dataset_paths[dataset]['semantic']
    ...
    def __getitem__(self, idx):
        # Load data
        if self.semantic:
            surface_path = os.path.splitext(self.file_paths.iloc[idx]['surface'])[0]+'.tif'
            overhead_path = os.path.splitext(self.file_paths.iloc[idx]['overhead'])[0]+'.tif'
            surface_raw = tifffile.imread(surface_path)
            overhead_raw = tifffile.imread(overhead_path)
...
class ImageNormalization(object):
    ...
    """
    def __init__(self, mean=Globals.img_mean, std=Globals.img_std):
...
class FOV_DSM(torch.nn.Module):
    ...
    def __init__(self, circ_padding=False):
        ...
        old_weight = model.features[0].weight.data
        model.features[0] = torch.nn.Conv2d(5, 64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))
        model.features[0].weight.data[:,:3,:,:] = old_weight

        # only train last 6 conv layers
        for name, param in model.features.named_parameters():
            torch_layer_num = int(name.split('.')[0])
            if torch_layer_num < 17 and torch_layer_num != 0:
...
def train(dataset='cvusa', fov=360, val_quantity=1000, batch_size=32, num_workers=8, num_epochs=999999):
